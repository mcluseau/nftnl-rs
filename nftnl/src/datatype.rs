...
/// Increments a big-endian byte slice by one.
///
/// Returns `None` if the value is already the maximum representable value.
pub(crate) fn increment_be(bytes: &[u8]) -> Option<Box<[u8]>> {
    let mut out = bytes.to_vec().into_boxed_slice();
